...
pub struct Limit {
    ...
    #[env_config(name = "ZO_S3_DOWNLOAD_NUM", default = 0)]
    pub s3_download_num: usize,
...
fn check_limit_config(cfg: &mut Config) -> Result<(), anyhow::Error> {
    ...
    if cfg.limit.s3_download_num == 0 {
        if cfg.common.local_mode {
            cfg.limit.s3_download_num = cpu_num * 2;
        } else {
            cfg.limit.s3_download_num = cpu_num * 4;
        }
    }
