...
import { S3Client, GetObjectCommand, PutObjectCommand, ListObjectsCommand, CopyObjectCommand, HeadObjectCommand, ListBucketsCommand, DeleteObjectsCommand, ListObjectsV2CommandOutput, ListObjectsV2Command } from '@aws-sdk/client-s3';
...
export class RemoteSnapshotLocation implements SnapshotLocation {
  ...
  async checkVersionExists(): Promise<boolean> {
    console.log(
      "RemoteSnapshot: checkVersionExists- this.version",
      this.version
    );
    ...
    try {
      ...
      console.error(
        `Error checking version existence: ${(error as Error).message}`
      );
      throw error;
    }
  }
  ...
  async backupVersion(): Promise<void> {
    ...
    try {
      ...
      if (response.Contents && response.Contents.length > 0) {
        console.log(
          `RemoteSnapshot: backupVersion- Backing up version ${this.version}...`
        );

        const copyPromises = response.Contents.map(async (item) => {
          if (item.Key) {
            console.log(
              `RemoteSnapshot: backupVersion- Backing up: ${item.Key}`
            );
            ...
            if (sourceKey.endsWith("/")) {
              console.log(
                `RemoteSnapshot: backupVersion- Skipping folder: ${sourceKey}`
              );
            ...
            console.log(
              `RemoteSnapshot: backupVersion- Copying from: ${this.bucket}/${sourceKey} to: ${backupKey}`
            );
            ...
            try {
              ...
              console.log(
                `RemoteSnapshot: backupVersion- Backed up: ${sourceKey} to ${backupKey}`
              );
            } catch (copyError) {
              console.error(
                `RemoteSnapshot: backupVersion- Error copying ${sourceKey} to ${backupKey}:`,
                copyError
              );
              throw copyError;
            }
        ...
        console.log(
          `RemoteSnapshot: backupVersion-Deleting old version ${this.version}...`
        );
        ...
        try {
          ...
          console.log(
            `RemoteSnapshot: backupVersion - Successfully deleted old version ${this.version}.`
          );
        } catch (deleteError) {
          console.error(
            `RemoteSnapshot: backupVersion - Error deleting old version ${this.version}:`,
            deleteError
          );
          throw deleteError;
        }
        ...
        console.log(
          `RemoteSnapshot: backupVersion- No items found to backup for version ${this.version}.`
        );
      }
    } catch (error) {
      console.error(
        `RemoteSnapshot: backupVersion- Error backing up version: ${
  ...
  // a custom function to fetch remote snapshots for a particular version (specified in the config file) from S3 bucket
      ...
      if (response.Contents) {
        for (const obj of response.Contents) {
          if (obj.Key) {
            ...
            // pushing the snapshot's name in an array
            items.push(fileName);
          }
        }
      }
